...
> **⚠️ IMPORTANTE:** Os gráficos abaixo serão gerados automaticamente quando você executar `python executar_completo.py`. Eles não existem no repositório até você rodar o código!

Após executar o código, 5 gráficos profissionais (PNG, 300 DPI) serão criados em `results/plots/`:
...
- **GD (vermelho):** Zig-zag entre paredes do vale
- **NR (azul):** Caminho direto ao mínimo

### 2. Curvas de Nível (`grafico_2_curvas_nivel.png`)
Vista superior evidenciando o comportamento diferencial dos métodos.

### 3. Convergência (`grafico_3_convergencia.png`)
Análise quantitativa em escala logarítmica mostrando convergência linear vs quadrática.

### 4. Comparação de Desempenho (`grafico_4_comparacao_desempenho.png`)
4 subplots comparando iterações, tempo, erro e custo computacional.

### 5. Sensibilidade ao α (`grafico_5_sensibilidade_alpha.png`)
Demonstra a fragilidade do GD em relação ao learning rate.
